...
export class TypecastTTSProvider implements ITTSProvider {
  ...
  async generateAudio(text: string, character: string): Promise<string> {
    ...
    console.log(`🎙️ Generating TTS (Typecast - ${character}): "${text.substring(0, 20)}..."`);
    ...
    try {
      ...
      if (!downloadUrl) {
        ...
        // 여기서는 바로 URL을 준다고 가정 (또는 speakResponse.data.result.download_url 등)
         throw new Error("Audio URL not returned immediately. Polling implementation might be required.");
      }
      ...
      console.error("❌ Failed to generate audio via Typecast:", error.response?.data || error.message);
      throw error;
    }
  }
}
