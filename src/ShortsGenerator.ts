...
export class ShortsGenerator {
  ...
  async generate(count: number = 1) {
    ...
    // 1. 질문 생성
    const questions =
      await this.config.questionGenerator.generateQuestions(count);
    ...
    for (const question of questions) {
      try {
        ...
        const [imgA, imgB, audio] = await Promise.all([
          ...
          this.config.ttsProvider.generateAudio(
            `${question.optionA}와 ${question.optionB}, 당신의 선택은?`,
            '박창수',
          ),
        ]);
        ...
        const frame = await this.config.frameComposer.composeFrame(
          question,
          imgA,
          imgB,
        );
        ...
        const outputVideoPath = path.join(
          ...
          `shorts_${question.id}.mp4`,
        );
        await this.config.videoRenderer.renderVideo(
          frame,
          audio,
          outputVideoPath,
        );
        ...
        console.error(
          `❌ Failed to generate shorts for question ${question.id}:`,
          error,
        );
      }
    }
