...
async function main() {
  ...
  if (!fs.existsSync(dummyFramePath)) {
    ...
    try {
      execSync(
        `ffmpeg -y -f lavfi -i color=c=red:s=1080x1920 -frames:v 1 "${dummyFramePath}"`,
        { stdio: 'ignore' },
      );
    } catch (error: unknown) {
      console.error(
        'Failed to create dummy frame via ffmpeg. Make sure ffmpeg is installed.',
        error,
      );
      return;
    }
  ...
  if (!fs.existsSync(dummyAudioPath)) {
    ...
    try {
      execSync(
        `ffmpeg -y -f lavfi -i anullsrc=r=44100:cl=stereo -t 5 -acodec libmp3lame "${dummyAudioPath}"`,
        { stdio: 'ignore' },
      );
    } catch (error: unknown) {
      console.error('Failed to create dummy audio via ffmpeg.', error);
      return;
    }
  }
