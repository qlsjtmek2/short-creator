...
async function testTypecast() {
  ...
  try {
    ...
    console.log("🎙️ Available Actors Count:", response.data.actors?.length || 0);
  } catch (error: any) {
    // 401이면 키 문제, 404면 엔드포인트 문제
    console.error("❌ Typecast API Error:", error.response?.status, error.response?.data || error.message);
  }
}
