...
### 작업 목록
- [x] README.md 작성 (설치, 사용법, 환경설정 가이드)
- [x] API 키 설정 가이드 문서화
- [x] 문제 해결 가이드 (troubleshooting)
...
### 기술 선택 이유 (최종)
1.  **Google Gemini**: 무료 티어 활용 가능, 한국어 처리 우수
2.  **GitHub Actions**: 코드 품질 유지, 배포 자동화 표준
3.  **Canvas & FFmpeg**: 서버 사이드 고성능 미디어 처리
4.  **ElevenLabs/Typecast**: 고품질 TTS 지원 (Mock 폴백 포함)
