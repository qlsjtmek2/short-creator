...
const TOPIC_POOL = [
  ...
  { category: '상상', text: '만약 지구가 멈춘다면?', color: 'bg-indigo-500/20 text-indigo-300 border-indigo-500/30' },
  { category: '역사', text: '조선시대 왕들의 비밀', color: 'bg-amber-500/20 text-amber-300 border-amber-500/30' },
  { category: '동물', text: '세상에서 가장 귀여운 동물 TOP 5', color: 'bg-pink-500/20 text-pink-300 border-pink-500/30' },
  { category: '음식', text: '한국인이 가장 사랑하는 야식 월드컵', color: 'bg-orange-500/20 text-orange-300 border-orange-500/30' },
  { category: '여행', text: '죽기 전에 꼭 가봐야 할 여행지', color: 'bg-teal-500/20 text-teal-300 border-teal-500/30' },
];

export default function Step1_Topic({ onNext, setLoading, isLoading, onOpenSettings }: Step1Props) {
  const [topic, setTopic] = useState('');
  const [recommendedTopics, setRecommendedTopics] = useState(TOPIC_POOL.slice(0, 5));
        ...
        <div className="w-full space-y-8">
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2 text-zinc-500 text-sm font-medium pl-1">
              ...
              <span>오늘의 추천 주제</span>
              <button 
                onClick={() => setRecommendedTopics([...TOPIC_POOL].sort(() => 0.5 - Math.random()).slice(0, 5))}
            ...
            <div className="flex flex-wrap gap-3">
              {recommendedTopics.map((item, idx) => (
                ...
                >
                  <span className="opacity-70 text-xs mr-2 font-bold">#{item.category}</span>
                  {item.text}
                </button>
              ))}
            </div>
